...
// Option de sélecteur de modèle : value = "providerID/modelID" (interne), label
// = nom LISIBLE (« OdyssAI-x · GLM 5.3 Flash ») — le préfixe provider (coeos) n'est
// JAMAIS montré. Entrée vide = hériter du modèle de session.
...
export function AgentEditor(props: {
  ...
  // Sélecteur de modèle : tous les modèles servables (OdyssAI-x + Minimax…),
  // libellés lisibles, groupés par provider. Le défaut (vide) hérite du modèle
  // de session (top-level config).
  const models = useModels()
  // Groupé par provider pour un <select> natif à optgroups. MÊME source filtrée que
  // le picker du Chat (dialog-select-model:55) : models.visible() = les toggles de
  // Settings > Models. Sophie cure sa liste là — l'éditeur d'agent doit la respecter,
  // pas montrer le catalogue brut. Zéro modèle en dur.
  const providerGroups = createMemo<{ provider: string; items: ModelOpt[] }[]>(() => {
    const groups = new Map<string, { provider: string; items: ModelOpt[] }>()
    for (const m of models.list()) {
      if (!models.visible({ modelID: m.id, providerID: m.provider.id })) continue
      const g = groups.get(m.provider.id) ?? { provider: m.provider.name, items: [] }
      g.items.push({ value: `${m.provider.id}/${m.id}`, label: m.name })
  ...
  // Modèle sauvé mais masqué depuis dans Settings > Models : on l'affiche quand même
  // (marqué), sinon le select retomberait visuellement sur « Défaut » à tort.
  // DOIT être déclaré APRÈS `form` (le memo s'évalue immédiatement -> TDZ sinon).
  const currentHidden = createMemo(() => {
    if (!form.model) return ""
    return providerGroups().some((g) => g.items.some((i) => i.value === form.model)) ? "" : form.model
  })
      ...
      <div class="mx-auto flex w-full max-w-[760px] flex-col gap-6 px-8 pb-10 pt-8">
        ...
        <div class="flex flex-col gap-5">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <FieldV2>
              <FieldV2.Label tooltip="Modèle qui fait tourner cet agent. Vide = modèle de session par défaut.">
                Model
              </FieldV2.Label>
              <select
                class="agent-model-select"
                data-action="agent-model"
                style={{ width: "100%", padding: "6px 8px", "border-radius": "6px" }}
                value={form.model}
                disabled={native()}
                onChange={(event) => setForm("model", event.currentTarget.value)}
              >
                {/* `selected` posé sur l'option (pas seulement value sur le select) :
                    Solid applique value AVANT d'insérer les options du <For> -> le
                    navigateur retombait sur la 1re option et la sélection sauvée
                    s'affichait « Défaut » (bug vécu 2026-08-29, serveur OK). */}
                <option value="" selected={!form.model}>
                  Défaut (modèle de session)
                </option>
                <Show when={currentHidden()}>
                  <option value={currentHidden()} selected>
                    {currentHidden().split("/").slice(1).join("/")} (masqué dans Settings)
                  </option>
                </Show>
                <For each={providerGroups()}>
                  {(g) => (
                    <optgroup label={g.provider}>
                      <For each={g.items}>
                        {(m) => (
                          <option value={m.value} selected={m.value === form.model}>
                            {m.label}
                          </option>
                        )}
                      </For>
