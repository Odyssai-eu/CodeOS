...
const DEFAULTS = {
  ...
  memoryFile: ".codeos/MEMORY.md", // mémoire persistante par projet (chargée via instructions)
  memoryMaxLines: 80,
}
...
export default {
  ...
  server: async (input, options) => {
    ...
    // Pas d'endpoint résolu (engine non appairé) -> plugin inerte, pas de hardcode.
    if (!opts.baseURL) {
      return { event: async () => {} }
    }
    ...
    async function updateMemory(info, messages) {
      ...
      // Crée le dossier du memoryFile CONFIGURÉ (ex. .odybench/), pas un .codeos figé :
      // sinon write=lit divergent (Odybench lit .odybench/, écrivait .codeos/).
