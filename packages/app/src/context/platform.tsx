...
type PlatformBase = {
  ...
  /** Get the model id assigned to the bench-judge agent (defaults to GLM 5.2 when unset) (desktop only) */
  getJudgeModel?(): Promise<string> | string

  /** Assign a model to the bench-judge agent — bare id from /v1/models (desktop only, restart-to-apply) */
  setJudgeModel?(modelID: string): Promise<void> | void

  /** Get the raw pasted CoeOS provider config (opencode config fragment JSON, "" if none) (desktop only) */
  getCoeosProvider?(): Promise<string> | string

  /** Set the CoeOS provider config from pasted JSON; validated + persisted to a file, "" clears it (desktop only, restart-to-apply) */
  setCoeosProvider?(text: string): Promise<{ ok: boolean; error?: string }> | { ok: boolean; error?: string }
