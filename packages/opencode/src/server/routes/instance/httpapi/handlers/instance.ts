...
export const instanceHandlers = HttpApiBuilder.group(InstanceHttpApi, "instance", (handlers) =>
  Effect.gen(function* () {
    ...
    const agentSave = Effect.fn("InstanceHttpApi.agentSave")(function* (ctx: {
      ...
      // Odybench (2026-08-28) : écrire l'agent en GLOBAL (comme skillSave), pas dans
      // le worktree. Un harnais de bench n'a pas de projet -> instance.worktree est
      // absent/non inscriptible -> "Unexpected server error". Global.Path.config/agent
      // est scanné par ConfigAgent.load via config.directories() et fusionné dans
      // result.agent (le model du markdown survit : le config injecté n'a plus de model).
