...
import { BUILD_MODEL_KEY, JUDGE_MODEL_DEFAULT, JUDGE_MODEL_KEY, PINCH_ZOOM_ENABLED_KEY, PLAN_MODEL_KEY, WINDOW_IDS_KEY, YOLO_KEY } from "./store-keys"
...
// Juge : défaut GLM 5.2 quand rien n'est stocké (le picker et l'agent partent
// donc de GLM, pas de CoeOS, sans que l'utilisatrice ait à choisir).
export function getJudgeModel(): string {
  const v = getStore().get(JUDGE_MODEL_KEY)
  return typeof v === "string" && v ? v : JUDGE_MODEL_DEFAULT
}

export function setJudgeModel(modelID: string) {
  getStore().set(JUDGE_MODEL_KEY, modelID)
}
