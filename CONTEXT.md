...
## coeos-code Orchestration (fork-local — section propre au fork, ne pas merger vers upstream)

_Terms résolus au grill du 2026-07-16. Le glossaire au-dessus est celui d'upstream (session runtime) ; cette section couvre la couche orchestration du fork coeos-code._

**CoeOS Mode**:
The third primary session mode (alongside Build and Plan) in which the Orchestrator plans work into a Task File and dispatches each task to role-scoped subagents.
_Avoid_: auto mode, pipeline mode, superagent mode

**Orchestrator**:
The primary agent active in CoeOS Mode. It plans, requests the single global GO, dispatches tasks, and tracks the Task File. The only coeos-code agent holding the `task` permission.
_Avoid_: coordinator, manager, superagent

**Executor**:
The write-enabled subagent that implements exactly one task from the Task File. Reachable only through CoeOS Mode dispatch.
_Avoid_: worker, builder

**Task File**:
The per-plan markdown checklist (`task.md`) that is the durable source of truth for planned work. Hand-editable mid-run; the Orchestrator re-reads it before every dispatch.
_Avoid_: todo list (session-scoped `todowrite` state), plan file (free-form prose)

**Mission Control**:
The dedicated full-screen desktop view showing the Task File, the active agents (role, model, status), and the selected agent's live transcript including reasoning.
_Avoid_: dashboard

**Schedule**:
A filesystem-declared recurring agent run (`.opencode/schedules/*.md`: cron expression + agent + prompt) executed by the coeos-code server.
_Avoid_: cron job, scheduled task

**Routing Brain**:
The shared score-table plus console role assignment (console `:4800`) that maps roles to servable model ids. Consumed by both coeos-code and the Superagent — « deux harnais, un cerveau ».
_Avoid_: model router, decider

**Sequencer**:
The coeos-code plugin that enforces triage, rigor floors, and forced review in code, independent of model judgment. The backstop beneath the Orchestrator.
_Avoid_: hook script

**Superagent**:
The headless batch runner living in the CoeOS repo. Shares the Routing Brain with coeos-code; neither ever calls the other.
_Avoid_: pipeline-as-model, CoeOS engine (that is the inference engine)
